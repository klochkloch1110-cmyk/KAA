...
export function DocumentsView() {
  const { documents, dataMode } = useAppStore();
  ...
  const sourceDocuments = dataMode === "supabase" ? documents : mockDocuments;
  ...
  const consignmentNoteCount = sourceDocuments.filter((d) => d.type === "consignment_note").length;
  const supportingCount = sourceDocuments.filter((d) => d.type === "supporting").length;
  const ocrIssuesCount = sourceDocuments.filter((d) => d.ocrStatus === "mismatch" || d.ocrStatus === "failed").length;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-card rounded-xl shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col">
                ...
                <div className="bg-card rounded-lg border border-border p-4 max-w-3xl w-full">
                  <div className="aspect-[3/4] bg-muted/50 rounded-lg flex items-center justify-center overflow-hidden">
                    {selectedDocument.fileUrl ? (
                      <img src={selectedDocument.fileUrl} alt="Предпросмотр документа" className="w-full h-full object-contain" />
                    ) : (
            ...
            <div className="px-6 py-4 border-t border-border bg-muted/30 flex items-center justify-between">
              <button onClick={() => openDocument(selectedDocument)} className="px-5 py-2.5 border border-border rounded-lg text-card-foreground hover:bg-muted transition-colors flex items-center gap-2">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => downloadDocument(selectedDocument)} className="px-5 py-2.5 bg-primary text-primary-foreground rounded-lg font-medium hover:opacity-90 transition-opacity shadow-md flex items-center gap-2">
                  <Download className="w-4 h-4" />
                  Скачать
                </button>
              </div>
