...
export function DriversView() {
  ...
  async function handleDriverCreated(d: CreatedDriver) {
  ...
  const visibleDrivers = dataMode === "supabase" ? supabaseDrivers : drivers;
  const visibleVehicles = dataMode === "supabase" ? supabaseVehicles : vehicles;
  const activeDrivers = visibleDrivers.filter((d) => d.status === "active").length;
  const totalTrips = visibleDrivers.reduce((s, d) => s + d.tripsThisMonth, 0);
  const totalEarnings = visibleDrivers.reduce((s, d) => s + d.earningsThisMonth, 0);
  const activeVehicles = visibleVehicles.filter((v) => v.status === "active").length;
    ...
    <div className="flex-1 overflow-auto bg-background">
        ...
        {tab === "drivers" && (
          <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
              <StatCard icon={<User className="w-5 h-5 text-primary" />} label="Всего водителей" value={String(visibleDrivers.length)} />
      ...
      {showCreateDriver && (
        <CreateDriverModal
          ...
          onCreated={(driver) => handleDriverCreated(driver)}
        />
      )}
      {showCreateVehicle && (
        <CreateVehicleModal
