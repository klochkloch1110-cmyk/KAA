...
export function TripsView() {
  ...
  const tripRows: Trip[] = trips.map((trip) => ({
    id: trip.id,
    orderId: trip.orderNumber,
    orderCustomer: trip.orderCustomer,
    driver: trip.driver,
    vehicle: trip.vehicle,
    date: trip.date,
    time: trip.time,
    consignmentNoteDriver: trip.consignmentNoteDriver,
    consignmentNoteOCR: trip.consignmentNoteOCR,
    volumeDriver: trip.volumeDriver,
    volumeOCR: trip.volumeOCR,
    status: trip.status,
    ocrStatus: trip.ocrStatus,
  }));
  ...
  const needsReviewCount = tripRows.filter((t) => t.status === "needs_review").length;
  const pendingOCRCount = tripRows.filter((t) => t.ocrStatus === "pending").length;
  const verifiedCount = tripRows.filter((t) => t.status === "verified").length;
