...
export function CreateOrderModal({ onClose, onCreated }: CreateOrderModalProps) {
  ...
  function validate(): boolean {
    ...
    if (form.volume && Number(form.volume) < 0) e.volume = "Объём не может быть отрицательным";
  ...
  async function handleSubmit() {
    ...
    try {
      await withTimeout(onCreated?.({
        ...
        volumeUnit: form.volumeUnit === "tons" ? "тонн" : "м³",
        pointA: form.pointA.trim(),
        pointB: form.pointB.trim(),
        ratePerTrip: Number(form.driverRate),
        clientRate: Number(form.clientRate),
        clientRateUnit: form.clientRateUnit,
        note: form.note.trim() || undefined,
      }) ?? Promise.resolve(), 15000, "Supabase не ответил за 15 секунд. Заявка не сохранена, попробуйте ещё раз.");
      setDone(true);
                  ...
                  <Field label="Общий объём" error={errors.volume}>
                    <div className="flex gap-2">
                      ...
                      <div className="relative">
                        ...
                        >
                          <option className="bg-white text-slate-900" value="tons">тонн</option>
                          <option className="bg-white text-slate-900" value="m3">м³</option>
                        </select>
                  ...
                  <Field label="Сумма, ₽ за единицу" error={errors.clientRate}>
                    <div className="flex gap-2">
                      ...
                      <div className="relative">
                        ...
                        >
                          <option className="bg-white text-slate-900" value="тонн">/ тонн</option>
                          <option className="bg-white text-slate-900" value="м³">/ м³</option>
                          <option className="bg-white text-slate-900" value="час">/ час</option>
                        </select>
