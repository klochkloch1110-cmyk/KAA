...
import type { AppShift } from "../store/AppStore";

export interface ShiftCloseOrderSummary {
  number: string;
  pointA: string;
  pointB: string;
  trips: number;
...
interface ShiftCloseModalProps {
  ...
  onClose: () => void;
  shift?: AppShift;
  orders?: ShiftCloseOrderSummary[];
  onSubmitted?: (input: { closingOdometer: number; fuelFilledLiters?: number }) => Promise<void> | void;
}
...
export function ShiftCloseModal({ isOpen, onClose, shift, orders = [], onSubmitted }: ShiftCloseModalProps) {
  const [mileage, setMileage] = useState("");
  const [fuel, setFuel]     = useState("");
  const [mileageErr, setMileageErr] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const [done, setDone]     = useState(false);
  const totalTrips = orders.reduce((sum, order) => sum + order.trips, 0);
  const totalVolume = orders.reduce((sum, order) => sum + order.volume, 0);
  const volumeUnit = orders.find((order) => order.volume > 0)?.volumeUnit ?? "тонн";
  ...
  async function handleSubmit() {
    if (!shift || !validateMileage(mileage)) return;
    setSubmitting(true);
    try {
      await onSubmitted?.({
        closingOdometer: Number(mileage),
        fuelFilledLiters: fuel ? Number(fuel) : undefined,
      });
      setDone(true);
      ...
      <div className="relative w-full max-w-lg bg-background rounded-t-2xl sm:rounded-2xl shadow-2xl max-h-[94vh] flex flex-col overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-primary to-accent px-5 py-4 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/70">{shift ? `${shift.date} · Начало ${shift.startTime}` : "Открытая смена не найдена"}</p>
            </div>
          </div>
          <button
            onClick={handleClose}
              ...
              <div className="px-4 py-3 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xl font-bold text-accent">
                    {totalVolume.toLocaleString("ru-RU")} <span className="text-base font-semibold text-muted-foreground">{volumeUnit}</span>
                  </span>
                </div>
        ...
        <div className="px-5 py-4 border-t border-border bg-muted/20 flex-shrink-0 flex flex-col gap-2">
          <button
            ...
            disabled={!shift || submitting || !mileage.trim() || !!mileageErr}
