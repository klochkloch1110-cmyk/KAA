...
export function DriverOrders() {
  const { user } = useAuth();
  const { getDriverOrders } = useAppStore();
  const orders: Order[] = getDriverOrders(user?.fullName ?? "Водитель", user?.id).map((order) => ({
    id: order.number,
    ...
    status: order.status,
    tripsCompleted: order.tripsCompleted,
    tripsTotal: order.tripsTotal,
  }));
